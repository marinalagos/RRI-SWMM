...
def compute_flow(opening_type, h1, h2, z, c, l, a = 0.15):
    """
    Computes the flow through a orifice or a weir following the equations in SWMM Hydraulics Reference Manual.
    Assumes the following shapes:
    - orifices: rectangular
    - weirs: side flow rectangular

    Args:
        opening_type (str): orifice' or 'weir'
        h1 (float): hydraulic head upstreams (m)
        h2 (float): hydraulic head downstreams (m)
        z (float): elevation of the bottom of the orifice opening / elevation of the weir's crest (m)
        c (float): discharge coefficient for orifice (dimensionless) / weir (m^0.5/s)
        l (float): length (m)
        a (float): only for orifices, height of its opening (m)

    Returns:
        float: flow discharge (m³/s)
    """
    reverse_flow = False
    ...
    if opening_type == 'orifice':
        ...
        # Compute the elevation of the top of the orifice opening
        h_top = z + a

        # Compute equivalent weir constant
        Cw_L = c * l * math.sqrt(g)

        # Check if there’s reverse flow 
        if h1 < h2:
            reverse_flow = True
            h1, h2 = h2, h1
        
        # Check if there's flow
        if h1 <= z:
            q = 0

        # Check if it works as a weir
        elif h1 < h_top: 
            q = Cw_L * (h1 - z)**(3/2)
            
            # Check if it is submerged downstreams
            if h2 > z:
                fs = (1 - ((h2-z)/(h1-z))**(3/2))**0.385
                q = fs * q

        # Compute the flow with the orifice discharge formula
        else: # h1 >= h_top

            # Compute the effective head
            if h2 < (z + a/2):
                he = h1 - (z + a/2)
            else:
                he = h1 - h2

            q = c * l * a * math.sqrt(2 * g * he)

        if reverse_flow:
            q = -q

    elif opening_type == 'weir':
        ...
        if h1 <= z:
            ...
            # Compute flow with side rectangular weir formula
            if not reverse_flow:
                q = c * (l**0.83) * (he**(5/3))
            else:
                q = c * l * (he**(3/2))
            ...
            if h2 > z:
                fs = (1 - ((h2-z)/(h1-z))**(5/3))**0.385
                q = fs * q

    else:
        print('"opening_type" should be "orifice" or "weir"')


    return q

def compute_flow_rri(opening_type, h1, h2, z, c, l, a = 0.15):
    """
    Computes the flow through a orifice or a weir following the equations in SWMM Hydraulics Reference Manual.
    Assumes the following shapes:
    - orifices: rectangular
    ...
    if opening_type == 'orifice':
        ...
        if h2 > z:
            ...
            if h2 > h1:
                # Case C
                reverse_flow = True
                h1, h2 = h2, h1 # on this equation h1 is the greatest
        
            if (h2-z)/(h1-z) <= (2/3):
                q = mu2 * (h1 - z) * math.sqrt(2 * g * (h1-z)) * l
            else:
                q = mu3 * (h2 - z) * math.sqrt(2 * g * (h1 - h2)) * l

        else:
            # Case A
            if h1 > z:
                q = mu1 * (h1 - z) * math.sqrt(g * (h1 - z)) * l
            else:
                q = 0
